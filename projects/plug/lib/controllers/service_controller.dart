...
class ServiceController extends GetxController {
  ...
  final _requestRef = FirebaseFirestore.instance.collection("request");
  RxList<ServiceModel> services = RxList([]);
  RxList<ServiceModel> fetchedServices = RxList([]);
  RxList<String> searchedSubCategories = RxList([]);
  RxList<Work> myJobs = RxList([]);
  RxList<Request> request = RxList([]);
  RxList<Request> selectedRequests = RxList([]);
  ...
  RxBool fetchingRequests = RxBool(false);
  ...
  Future<void> getAllServices({String? name}) async {
    try {
      ...
      if (allData.isNotEmpty) {
        services.assignAll(allData.map((e) => ServiceModel.fromJson(e)).toList());
        fetchedServices.assignAll(allData.map((e) => ServiceModel.fromJson(e)).toList());
      } else {
        services.value = [];
   ...
   checkLocation({ bool proceed=true}) async {
    loc.Location location = loc.Location();
    bool serviceEnabled = await location.serviceEnabled();
    if (!serviceEnabled) {
      Get.off(() => const LocationAllow());
    } else {
  ...
  getWorkHistoryByUserId(
    ...
    try {
      ...
      if (checkByProvider == true) {
        ...
        await query.orderBy("position").get().then((value) {
  ...
  getRequestsByProviderId(
    ...
    try {
      ...
      fetchingRequests.value = true;
        var query = _requestRef.where("provider.id", isEqualTo: id);
        await query.orderBy("position").get().then((value) {
          if (value.docs.isNotEmpty) {
            for (var element in value.docs) {
              Request requestData = Request.fromJson(element.data());
  ...
  Future capturePayment(
    ...
    try {
     ...
     finishTransaction(uid: uid, userModel: userModel, chatData: chatData);
      }
  ...
  Future finishTransaction(
      ...
      ChatController chatController = Get.find<ChatController>();
        Chat chat = Chat(
          message:
              "Thank you for choosing plug, we hope you had a great experience.Please make sure to leave a review for ${userModel.firstname}.Reviews help us make sure this community remains a safe.",
          senderInboxMessage:
              "Thank you for choosing plug, we hope you had a great experience.Please make sure to leave a review for ${userModel.firstname}.Reviews help us make sure this community remains a safe.",
          receiverInboxMessage:
              "Thank you for choosing plug, we hope you had a great experience.Please make sure to leave a review for ${Get.find<AuthController>().currentUser.value!.firstname}.Reviews help us make sure this community remains a safe.",
          type: "offer",
  ...
  searchService({required String text}) {
    ...
    List<ServiceModel> searchResults = services.where((item) {
      bool nameMatches = item.name!.contains(text);
      List<String>? subcategoryMatches = item.subcategory
          ?.where((subcategory) => subcategory.toLowerCase().contains(text))
          .toList();
      print("search is ${subcategoryMatches}");
      if (subcategoryMatches!.isNotEmpty) {
        searchedSubCategories.assignAll(subcategoryMatches);
      }

      // Return true if either the name or any subcategory matches
      return nameMatches;
    }).toList();

    print("results are ${searchResults}");
    fetchedServices.assignAll(searchResults);
    for (var i = 0; i < searchedSubCategories.length; i++) {
      ServiceModel serviceModel = ServiceModel(
        name: searchedSubCategories[i],
        id: i.toString(),
        icon: "",
        expand: false,
        isSubcategory: true,
      );
      fetchedServices.add(serviceModel);
    }

    fetchedServices.refresh();
  }
  ...
  void payWithFlutterWave(
    ...
    try {
      ...
      if (response.success == true) {
        var id=await Get.find<ChatController>()
            .createRequest(userModel: userModel, price: chatData.hours! * chatData.price!, uid: uid);
        acceptOffer(
          requestId: id,
            userModel: userModel,
            chatData: chatData,
  ...
  acceptOffer({
    ...
    Chat chat = Chat(
      message: "Dear ${authController.currentUser.value!.firstname}, Thank you for accepting ${userModel.firstname!} offer.${userModel.firstname!.toString()} is on their way to your current location now.If you would like them to arrive at a different location please let them know.",
      senderInboxMessage: "Dear ${authController.currentUser.value!.firstname}, Thank you for accepting ${userModel.firstname!} offer.${userModel.firstname!.toString()} is on their way to your current location now.If you would like them to arrive at a different location please let them know.",
      type: "offer",
      addTime: DateTime.now(),
      senderId: Get.find<AuthController>().currentUser.value,
      receiverId: userModel,
      ...
      requestId:requestId,
      finished: false,
      receiverMessage: "${authController.currentUser.value!.firstname} accepted your offer. They are now waiting for you. Please start heading over their way. ${authController.currentUser.value!.firstname} address is ${authController.currentUser.value!.address} \nOnce you arrived at client’s location, please confirm your arrival.\nTIP: It is best for you to confirm your client’s address before you start your drive.  ",
      receiverInboxMessage:
    ...
    chatController.updateChatById(
        uid: uid,
        chatId: chatData.id,
        body: {"finished": true, "action": "Offer Accepted","requestId":requestId});

    chatController.updateChatById(
        uid: uid, chatId: chatData.id, body: {"transactionId": transactionId,"requestId":requestId});
  ...
  void callBack(
    ...
    var id = await Get.find<ChatController>()
        .createRequest(userModel: userModel, price: amount, uid: uid);
    acceptOffer(
        requestId: id,
        userModel: userModel,
        chatData: chatData,
