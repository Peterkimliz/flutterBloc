...
class UserController extends GetxController with GetTickerProviderStateMixin {
  ...
  RxBool showFollowerSearch = RxBool(false);
  ...
  void searchUsersBasedOnService({String? name = ""}) async {
    ServiceController serviceController = Get.find<ServiceController>();
    filteredUsers.clear();
    try {
      fetchingUsers.value = true;
      Query queryies = firestoreReference;
      if (name!.isNotEmpty) {
        queryies = queryies
            .where('firstname', isGreaterThanOrEqualTo: name.toLowerCase(), isLessThan: '${name.toLowerCase()}z')
        ...
        if (serviceController.category.value!.isSubcategory == true) {
          queryies = queryies
              .where("service.subCategory",
          ...
          if (Get.find<AuthController>().currentUser.value != null) {
            queryies = queryies.where("id",isNotEqualTo: FirebaseAuth.instance.currentUser!.uid);
          }

          if (serviceController.category.value!.subCategory!
                  .trim()
                  .isNotEmpty &&
              serviceController.category.value!.subCategory!
                      .trim()
                      .toLowerCase() !=
                  "all") {
      ...
      QuerySnapshot query = await queryies.get();
      print("object ${query.docs.length}");
      if (query.docs.isNotEmpty) {
        for (var element in query.docs) {
          UserModel userModel = UserModel.fromJson(element.data() as Map<String, dynamic>);
          if (selectedDays.isNotEmpty && name.trim().isEmpty) {
            var contain = userModel.availability!.toSet().containsAll(selectedDays);
            if (contain) {
            ...
            filteredUsers.add(userModel);
            print("Query length two is ${query.docs.length}");
          }
        }

        List<UserModel> filteredUsersnew =name.trim().isNotEmpty==true?filteredUsers.toList():[];

        //     :
        // filteredUsers.where((element) {
        ...
        print("Query length three is ${filteredUsers.length}");
      } else {
        filteredUsers.value = [];
      }
  ...
  followUser({required UserModel userModel, String? type}) {
    ...
    authController.updateSingleItem(body: {"followersCount": FieldValue.increment(1)}, id: userModel.id!);
    authController.updateSingleItem(
        body: {"followingCount": FieldValue.increment(1)},
        id: FirebaseAuth.instance.currentUser!.uid);
  }
  ...
  getUserFollowers(
      {required String id, required String type, String? name}) async {
    try {
      loadingFollowers.value = true;

      List<UserModel> followers = [];
      Query query = firestoreReference.doc(id).collection(type);
      QuerySnapshot queryResult = await query.get();
      if (queryResult.docs.isNotEmpty) {
        if (name != null) {
          List searchedUsers=  queryResult.docs
              .where((item) => item["firstname"].toLowerCase().startsWith(name.toLowerCase()))
              .toList();
          for (var element in searchedUsers) {
            UserModel userModel =
            UserModel.fromJson(element.data() as Map<String, dynamic>);
